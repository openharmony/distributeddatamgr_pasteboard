...
/*
 * Copyright (c) 2026-2021 Huawei Device Co., Ltd.
...
{ // LCOV_EXCL_START
    ...
    auto originInfo = props_.additions.GetWantParams(ORIGIN_INFO);
    originInfo.SetParam(ORIGIN_TOKEN_ID, AAFwk::Integer::Box(tokenId));
...
std::string PasteData::CreatePasteId(const std::string &name, uint32_t sequence, pid_t pid)
{ // LCOV_EXCL_START
    std::string currentId = name + "_" + std::to_string(pid) + "_" + std::to_string(sequence);
    return currentId;
} // LCOV_EXCL_STOP
