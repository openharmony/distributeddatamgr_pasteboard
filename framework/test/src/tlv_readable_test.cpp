...
/**
 * @tc.name: ReadValueVariantTest001
 * @tc.desc: test ReadValue for std::variant with valid index 0
 * @tc.type: FUNC
 */
HWTEST_F(TLVReadableTest, ReadValueVariantTest001, TestSize.Level1)
{
    PASTEBOARD_HILOGI(PASTEBOARD_MODULE_CLIENT, "ReadValueVariantTest001 start");
    using VariantType = std::variant<int32_t, double>;
    size_t innerLen = sizeof(TLVHead) + sizeof(uint32_t) + sizeof(TLVHead) + sizeof(int32_t);
    std::vector<std::uint8_t> buffer(sizeof(TLVHead) + innerLen);
    size_t offset = 0;
    auto *outer = reinterpret_cast<TLVHead *>(buffer.data() + offset);
    outer->tag = 1;
    outer->len = static_cast<uint32_t>(innerLen);
    offset += sizeof(TLVHead);
    auto *indexHead = reinterpret_cast<TLVHead *>(buffer.data() + offset);
    indexHead->tag = TAG_VARIANT_INDEX;
    indexHead->len = sizeof(uint32_t);
    offset += sizeof(TLVHead);
    uint32_t index = 0;
    memcpy_s(buffer.data() + offset, sizeof(uint32_t), &index, sizeof(uint32_t));
    offset += sizeof(uint32_t);
    auto *valueHead = reinterpret_cast<TLVHead *>(buffer.data() + offset);
    valueHead->tag = TAG_VARIANT_VALUE;
    valueHead->len = sizeof(int32_t);
    offset += sizeof(TLVHead);
    int32_t expectedValue = 42;
    memcpy_s(buffer.data() + offset, sizeof(int32_t), &expectedValue, sizeof(int32_t));
    ReadOnlyBuffer buff(buffer);
    TLVHead head;
    ASSERT_TRUE(buff.ReadHead(head));
    VariantType value;
    bool res = buff.ReadValue(value, head);
    EXPECT_TRUE(res);
    EXPECT_EQ(value.index(), 0);
    EXPECT_EQ(std::get<0>(value), 42);
    PASTEBOARD_HILOGI(PASTEBOARD_MODULE_CLIENT, "ReadValueVariantTest001 end");
}

/**
 * @tc.name: ReadValueVariantTest002
 * @tc.desc: test ReadValue for std::variant with valid index 1
 * @tc.type: FUNC
 */
HWTEST_F(TLVReadableTest, ReadValueVariantTest002, TestSize.Level1)
{
    PASTEBOARD_HILOGI(PASTEBOARD_MODULE_CLIENT, "ReadValueVariantTest002 start");
    using VariantType = std::variant<int32_t, double>;
    size_t innerLen = sizeof(TLVHead) + sizeof(uint32_t) + sizeof(TLVHead) + sizeof(double);
    std::vector<std::uint8_t> buffer(sizeof(TLVHead) + innerLen);
    size_t offset = 0;
    auto *outer = reinterpret_cast<TLVHead *>(buffer.data() + offset);
    outer->tag = 1;
    ...
    auto *valueHead = reinterpret_cast<TLVHead *>(buffer.data() + offset);
    valueHead->tag = TAG_VARIANT_VALUE;
    valueHead->len = sizeof(double);
    offset += sizeof(TLVHead);
    double expectedValue = 3.14;
    memcpy_s(buffer.data() + offset, sizeof(double), &expectedValue, sizeof(double));
    ReadOnlyBuffer buff(buffer);
    TLVHead head;
    ASSERT_TRUE(buff.ReadHead(head));
    VariantType value;
    ...
    PASTEBOARD_HILOGI(PASTEBOARD_MODULE_CLIENT, "ReadValueVariantTest002 end");
}

/**
 * @tc.name: ReadValueVariantTest003
 * @tc.desc: test ReadValue for std::variant with out-of-bounds index
 * @tc.type: FUNC
 */
HWTEST_F(TLVReadableTest, ReadValueVariantTest003, TestSize.Level1)
{
    PASTEBOARD_HILOGI(PASTEBOARD_MODULE_CLIENT, "ReadValueVariantTest003 start");
    using VariantType = std::variant<int32_t, double>;
    size_t innerLen = sizeof(TLVHead) + sizeof(uint32_t);
    std::vector<std::uint8_t> buffer(sizeof(TLVHead) + innerLen);
    size_t offset = 0;
    auto *outer = reinterpret_cast<TLVHead *>(buffer.data() + offset);
    outer->tag = 1;
    ...
    bool res = buff.ReadValue(value, head);
    EXPECT_FALSE(res);
    PASTEBOARD_HILOGI(PASTEBOARD_MODULE_CLIENT, "ReadValueVariantTest003 end");
}

/**
 * @tc.name: ReadValueVariantTest004
 * @tc.desc: test ReadValue for std::variant with very large out-of-bounds index
 * @tc.type: FUNC
 */
HWTEST_F(TLVReadableTest, ReadValueVariantTest004, TestSize.Level1)
{
    PASTEBOARD_HILOGI(PASTEBOARD_MODULE_CLIENT, "ReadValueVariantTest004 start");
    using VariantType = std::variant<int32_t, double>;
    size_t innerLen = sizeof(TLVHead) + sizeof(uint32_t);
    std::vector<std::uint8_t> buffer(sizeof(TLVHead) + innerLen);
    size_t offset = 0;
    ...
    bool res = buff.ReadValue(value, head);
    EXPECT_FALSE(res);
    PASTEBOARD_HILOGI(PASTEBOARD_MODULE_CLIENT, "ReadValueVariantTest004 end");
}

/**
 * @tc.name: ReadValueEntryVariantTest001
 * @tc.desc: test ReadValue for EntryValue with out-of-bounds index
 * @tc.type: FUNC
 */
HWTEST_F(TLVReadableTest, ReadValueEntryVariantTest001, TestSize.Level1)
{
    PASTEBOARD_HILOGI(PASTEBOARD_MODULE_CLIENT, "ReadValueEntryVariantTest001 start");
    size_t innerLen = sizeof(TLVHead) + sizeof(uint32_t);
    std::vector<std::uint8_t> buffer(sizeof(TLVHead) + innerLen);
    size_t offset = 0;
    auto *outer = reinterpret_cast<TLVHead *>(buffer.data() + offset);
    ...
    bool res = buff.ReadValue(value, head);
    EXPECT_FALSE(res);
    PASTEBOARD_HILOGI(PASTEBOARD_MODULE_CLIENT, "ReadValueEntryVariantTest001 end");
}
