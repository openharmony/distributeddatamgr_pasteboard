...
bool ReadOnlyBuffer::ReadValue(std::string &value, const TLVHead &head)
{
    PASTEBOARD_CHECK_AND_RETURN_RET_LOGE(HasExpectBuffer(head.len), false,
        PASTEBOARD_MODULE_COMMON, "read string failed, tag=%{public}hu", head.tag);
...
{
    PASTEBOARD_CHECK_AND_RETURN_RET_LOGE(HasExpectBuffer(head.len), false,
        PASTEBOARD_MODULE_COMMON, "read RawMem failed, tag=%{public}hu", head.tag);
...
{
    PASTEBOARD_CHECK_AND_RETURN_RET_LOGE(HasExpectBuffer(head.len), false,
        PASTEBOARD_MODULE_COMMON, "read TLVReadable failed, tag=%{public}hu", head.tag);
    auto tmp = total_;
...
{
    PASTEBOARD_CHECK_AND_RETURN_RET_LOGE(HasExpectBuffer(head.len), false,
        PASTEBOARD_MODULE_COMMON, "read vector failed, tag=%{public}hu", head.tag);
...
bool ReadOnlyBuffer::ReadValue(std::shared_ptr<AAFwk::Want> &value, const TLVHead &head)
{
    RawMem rawMem{};
    bool ret = ReadValue(rawMem, head);
    value = TLVUtils::Raw2Parcelable<AAFwk::Want>(rawMem);
    return ret;
}
