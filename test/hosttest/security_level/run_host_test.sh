...
#
# Host-side build + run + coverage loop for the SecurityLevel module.
# Deep-dependency: sits behind the DEVSL C API (dataclassification) and DMAdapter
# (device_manager). Builds against fakes/ (DEVSL result/level + udid as test
# hooks). The fakes dir is FIRST on -I so it shadows the real platform headers.
#
...
SEC_INC="${PASTEBOARD_ROOT}/adapter/security_level"
SEC_SRC="${PASTEBOARD_ROOT}/adapter/security_level/security_level.cpp"
TEST_SRC="${SCRIPT_DIR}/security_level_host_test.cpp"

BUILD_DIR="${SCRIPT_DIR}/.build"
BIN="${BUILD_DIR}/security_level_host_test"
...
for f in "${GTEST_ROOT}/src/gtest-all.cc" "${SEC_SRC}" "${TEST_SRC}" \
         "${FAKES_INC}/dev_slinfo_mgr.h" "${FAKES_INC}/device/dm_adapter.h" \
         "${FAKES_INC}/pasteboard_hilog.h"; do
...
# fakes FIRST so they shadow real platform headers; then the module's own dir.
UUT_INC=(-I"${FAKES_INC}" -I"${SEC_INC}")
...
info "compiling security_level.cpp (WITH coverage, against fakes)"
( cd "${BUILD_DIR}" && "${CXX}" -c "${SEC_SRC}" "${UUT_INC[@]}" \
    -std=c++17 -O0 -g --coverage -o security_level.o ) \
...
info "linking"
"${CXX}" --coverage \
    "${BUILD_DIR}/test.o" "${BUILD_DIR}/security_level.o" \
...
COV_LINE="$( cd "${BUILD_DIR}" && "${GCOV}" -n security_level.gcno 2>/dev/null \
    | grep -A1 "security_level.cpp'" | grep "Lines executed" | head -1 )"
echo "  ${COV_LINE}"
LINE_COV="$(echo "${COV_LINE}" | grep -oE "[0-9]+\.[0-9]+" | head -1)"

[[ -n "${LINE_COV}" ]] || { fail "could not parse coverage output"; exit 3; }
info "security_level.cpp line coverage: ${LINE_COV}% (min ${COVERAGE_MIN}%)"
