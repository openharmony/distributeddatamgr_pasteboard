...
#
# Host-side build + run + coverage loop for the EventCenter module.
# See ../serializable/README.md for the pattern; this is the second instance,
# and the first that needs a "fake seam" (shim/pasteboard_hilog.h) to replace a
# device-only dependency.
#
# Single command:  ./run_host_test.sh
# Exit: 0 pass+coverage ok | 1 test fail | 2 coverage below gate | 3 build error
#
# Env overrides: COVERAGE_MIN (default 90), CXX (default g++), GCOV (gcov-12)
...
SHIM_INC="${SCRIPT_DIR}/shim"                                   # fake seam for hilog
EVENT_SRC="${PASTEBOARD_ROOT}/framework/framework/eventcenter/event.cpp"
CENTER_SRC="${PASTEBOARD_ROOT}/framework/framework/eventcenter/event_center.cpp"
TEST_SRC="${SCRIPT_DIR}/event_center_host_test.cpp"

BUILD_DIR="${SCRIPT_DIR}/.build"
BIN="${BUILD_DIR}/event_center_host_test"
...
for f in "${GTEST_ROOT}/src/gtest-all.cc" "${EVENT_SRC}" "${CENTER_SRC}" "${TEST_SRC}" \
         "${SHIM_INC}/pasteboard_hilog.h"; do
...
# Includes: shim FIRST so its pasteboard_hilog.h shadows the real one; then the
# framework headers (event.h, event_center.h, common/concurrent_map.h).
UUT_INC=(-I"${SHIM_INC}" -I"${FW_INC}")
...
info "compiling event.cpp + event_center.cpp (WITH coverage)"
( cd "${BUILD_DIR}" && \
  "${CXX}" -c "${EVENT_SRC}"  "${UUT_INC[@]}" -std=c++17 -O0 -g --coverage -o event.o && \
  "${CXX}" -c "${CENTER_SRC}" "${UUT_INC[@]}" -std=c++17 -O0 -g --coverage -o event_center.o ) \
...
info "linking"
"${CXX}" --coverage \
    "${BUILD_DIR}/test.o" "${BUILD_DIR}/event.o" "${BUILD_DIR}/event_center.o" \
...
# Combined line coverage across the two unit-under-test files:
# pull "executed% of N" for each file, weight by line count for a combined number.
total_lines=0
covered_lines=0
# gcov must be run from inside BUILD_DIR against the LOCAL .gcno basename (not
# the full source path, or it looks for notes files next to the source).
for gcno in event event_center; do
    src_base="${gcno}.cpp"
...
info "eventcenter combined line coverage: ${LINE_COV}% (min ${COVERAGE_MIN}%)"
