...
#
# Host-side build + run + coverage loop for the Command module (dfx CLI helper).
# Pure logic; no shim, no fake. See ../serializable/README.md for the pattern.
#
...
CMD_INC="${PASTEBOARD_ROOT}/services/dfx/src"
CMD_SRC="${PASTEBOARD_ROOT}/services/dfx/src/command.cpp"
TEST_SRC="${SCRIPT_DIR}/command_host_test.cpp"

BUILD_DIR="${SCRIPT_DIR}/.build"
BIN="${BUILD_DIR}/command_host_test"
...
for f in "${GTEST_ROOT}/src/gtest-all.cc" "${CMD_SRC}" "${TEST_SRC}"; do
...
info "compiling command.cpp (WITH coverage)"
( cd "${BUILD_DIR}" && "${CXX}" -c "${CMD_SRC}" "${UUT_INC[@]}" \
    -std=c++17 -O0 -g --coverage -o command.o ) \
...
info "linking"
"${CXX}" --coverage \
    "${BUILD_DIR}/test.o" "${BUILD_DIR}/command.o" \
...
COV_LINE="$( cd "${BUILD_DIR}" && "${GCOV}" -n command.gcno 2>/dev/null \
    | grep -A1 "command.cpp'" | grep "Lines executed" | head -1 )"
echo "  ${COV_LINE}"
LINE_COV="$(echo "${COV_LINE}" | grep -oE "[0-9]+\.[0-9]+" | head -1)"

[[ -n "${LINE_COV}" ]] || { fail "could not parse coverage output"; exit 3; }
info "command.cpp line coverage: ${LINE_COV}% (min ${COVERAGE_MIN}%)"
