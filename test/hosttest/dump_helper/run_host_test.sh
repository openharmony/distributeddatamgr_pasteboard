...
#
# Host-side build + run + coverage loop for PasteboardDumpHelper.
# Composition sample: links the real command.cpp (already host-tested). No shim,
# no fake. Coverage measured on pasteboard_dump_helper.cpp only.
#
...
DFX_INC="${PASTEBOARD_ROOT}/services/dfx/src"
DUMP_SRC="${PASTEBOARD_ROOT}/services/dfx/src/pasteboard_dump_helper.cpp"
CMD_SRC="${PASTEBOARD_ROOT}/services/dfx/src/command.cpp"
TEST_SRC="${SCRIPT_DIR}/dump_helper_host_test.cpp"

BUILD_DIR="${SCRIPT_DIR}/.build"
BIN="${BUILD_DIR}/dump_helper_host_test"
...
for f in "${GTEST_ROOT}/src/gtest-all.cc" "${DUMP_SRC}" "${CMD_SRC}" "${TEST_SRC}"; do
...
info "compiling command.cpp (dependency, no coverage)"
"${CXX}" -c "${CMD_SRC}" "${UUT_INC[@]}" -std=c++17 -O0 -g \
    -o "${BUILD_DIR}/command.o" || { fail "command compile failed"; exit 3; }
...
info "compiling pasteboard_dump_helper.cpp (WITH coverage)"
( cd "${BUILD_DIR}" && "${CXX}" -c "${DUMP_SRC}" "${UUT_INC[@]}" \
    -std=c++17 -O0 -g --coverage -o pasteboard_dump_helper.o ) \
...
info "linking"
"${CXX}" --coverage \
    "${BUILD_DIR}/test.o" "${BUILD_DIR}/pasteboard_dump_helper.o" "${BUILD_DIR}/command.o" \
...
COV_LINE="$( cd "${BUILD_DIR}" && "${GCOV}" -n pasteboard_dump_helper.gcno 2>/dev/null \
    | grep -A1 "pasteboard_dump_helper.cpp'" | grep "Lines executed" | head -1 )"
echo "  ${COV_LINE}"
LINE_COV="$(echo "${COV_LINE}" | grep -oE "[0-9]+\.[0-9]+" | head -1)"

[[ -n "${LINE_COV}" ]] || { fail "could not parse coverage output"; exit 3; }
info "pasteboard_dump_helper.cpp line coverage: ${LINE_COV}% (min ${COVERAGE_MIN}%)"
