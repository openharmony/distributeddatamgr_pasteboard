...
#
# Host-side build + run + coverage loop for ProgressSignalClient.
# The unit's only external include (distributed_file_daemon_manager.h) is unused,
# so it is shimmed empty; nocopyable.h comes from c_utils (header-only) via an
# include path. Shim dir is first on -I.
#
...
CUTILS_INC="${CODE_ROOT}/commonlibrary/c_utils/base/include"   # nocopyable.h
SHIM_INC="${SCRIPT_DIR}/shim"                                  # daemon-mgr + hilog
PS_INC="${PASTEBOARD_ROOT}/framework/innerkits/include"
FW_INC="${PASTEBOARD_ROOT}/framework/framework/include"        # api/visibility.h
PS_SRC="${PASTEBOARD_ROOT}/framework/innerkits/src/pasteboard_progress_signal.cpp"
TEST_SRC="${SCRIPT_DIR}/progress_signal_host_test.cpp"

BUILD_DIR="${SCRIPT_DIR}/.build"
BIN="${BUILD_DIR}/progress_signal_host_test"
...
for f in "${GTEST_ROOT}/src/gtest-all.cc" "${PS_SRC}" "${TEST_SRC}" \
         "${SHIM_INC}/distributed_file_daemon_manager.h" "${SHIM_INC}/pasteboard_hilog.h" \
         "${CUTILS_INC}/nocopyable.h"; do
...
# shim FIRST so it shadows the real daemon-mgr / hilog headers.
UUT_INC=(-I"${SHIM_INC}" -I"${PS_INC}" -I"${FW_INC}" -I"${CUTILS_INC}")
...
info "compiling pasteboard_progress_signal.cpp (WITH coverage)"
( cd "${BUILD_DIR}" && "${CXX}" -c "${PS_SRC}" "${UUT_INC[@]}" \
    -std=c++17 -O0 -g --coverage -o pasteboard_progress_signal.o ) \
...
info "linking"
"${CXX}" --coverage \
    "${BUILD_DIR}/test.o" "${BUILD_DIR}/pasteboard_progress_signal.o" \
...
COV_LINE="$( cd "${BUILD_DIR}" && "${GCOV}" -n pasteboard_progress_signal.gcno 2>/dev/null \
    | grep -A1 "pasteboard_progress_signal.cpp'" | grep "Lines executed" | head -1 )"
echo "  ${COV_LINE}"
LINE_COV="$(echo "${COV_LINE}" | grep -oE "[0-9]+\.[0-9]+" | head -1)"

[[ -n "${LINE_COV}" ]] || { fail "could not parse coverage output"; exit 3; }
info "pasteboard_progress_signal.cpp line coverage: ${LINE_COV}% (min ${COVERAGE_MIN}%)"
