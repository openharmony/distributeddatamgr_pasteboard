...
* @tc.desc: marshalling test: unMarshalling parcelable failed.
* @tc.type: FUNC
*/
HWTEST_F(PasteboardServiceTest, PasteDataTest008, TestSize.Level0)
{
    ...
    ASSERT_TRUE(secondRecord->GetMimeType() == MIMETYPE_TEXT_PLAIN);
    getPlainText = secondRecord->GetPlainText();
    ASSERT_TRUE(getPlainText != nullptr);
    ASSERT_TRUE(*getPlainText == plainText);
}
...
* @tc.desc: marshalling test: unMarshalling string failed.
* @tc.type: FUNC
*/
...
* @tc.desc: marshalling test: unMarshalling string and parcelable all failed.
* @tc.type: FUNC
*/
HWTEST_F(PasteboardServiceTest, PasteDataTest010, TestSize.Level0)
{
    std::string plainText = "helloWorld";
    auto pasteData = PasteboardClient::GetInstance()->CreatePlainTextData(plainText);
    ASSERT_TRUE(pasteData != nullptr);
