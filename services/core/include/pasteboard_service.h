...
public:
    ...
    ConcurrentMap<int32_t, ScreenEvent> screenStatusMap_;
    ...
    bool IsFileManagerApp(const std::string &bundleName, uint64_t tokenId);
    int32_t CheckRemoteFileDocsUriLimit(const std::vector<Uri> &grantUris, uint32_t targetTokenId, const std::string &bundleName);
...
#ifdef PB_COCKPIT_PLATFORM_ENABLE
    std::shared_ptr<PasteboardSubProfileSubscriber> subProfileSubscriber_ = nullptr;
#endif
...
#ifdef PB_COCKPIT_PLATFORM_ENABLE
    void SubProfileSubscriber();
    void SubProfileUnsubscriber();
