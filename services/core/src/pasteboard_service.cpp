...
#include "pasteboard_permission_manager.h"
#include "pasteboard_entity_recognition_manager.h"
...
{
    clients_.Clear();
    entityRecognitionMgr_->UnsubscribeAllEntityObserver();
}
...
void PasteboardService::OnStart()
{
    ...
    distributedMgr_ = std::make_shared<PasteboardDistributedManager>(*this);
    distributedMgr_->moduleConfig_.Init();
...
#endif
    distributedMgr_->moduleConfig_.Watch(std::bind(&PasteboardDistributedManager::OnConfigChange, distributedMgr_.get(), std::placeholders::_1));
    distributedMgr_->ffrtTimer_ = FFRTPool::GetTimer("pasteboard_service");
    UpdateAgedTime();
    distributedMgr_->Init();
    permissionMgr_ = std::make_shared<PasteboardPermissionManager>(*this);
    entityRecognitionMgr_ = std::make_shared<PasteboardEntityRecognitionManager>(*this);
    ...
    auto callback = [this]() {
        ...
        if (userId != ERROR_USERID) {
            distributedMgr_->switch_.Init(userId);
        }
    };
    ...
    EventCenter::GetInstance().Subscribe(OHOS::MiscServices::Event::EVT_REMOTE_CHANGE, distributedMgr_->RemotePasteboardChange());
...
void PasteboardService::OnStop()
{
    ...
    distributedMgr_->moduleConfig_.DeInit();
    distributedMgr_->switch_.DeInit();
#ifdef PB_DATACLASSIFICATION_ENABLE
    DATASL_OnStop();
#endif
...
{
    PASTEBOARD_CHECK_AND_RETURN_LOGE(distributedMgr_->ffrtTimer_ != nullptr, PASTEBOARD_MODULE_SERVICE, "ffrtTimer_ is null");
    distributedMgr_->ffrtTimer_->CancelTimer(SET_CRITICAL_ID);
...
void PasteboardService::SetCriticalTimer()
{
    ...
    distributedMgr_->ffrtTimer_->SetTimer(SET_CRITICAL_ID, task, static_cast<uint32_t>(agedTime_.load()));
...
{
    distributedMgr_->HandleWifiOffAndClearDistributedEvent(userId);
}
...
int32_t PasteboardService::ClearInner(int32_t userId, const AppInfo &appInfo)
{
    ...
    distributedMgr_->CleanDistributedData(userId);
    if (hasData) {
...
{
    entityRecognitionMgr_->NotifyEntityObservers(entity, entityType, dataLength);
}
...
{
    return entityRecognitionMgr_->SubscribeEntityObserver(entityType, expectedDataLength, observer);
}

int32_t PasteboardService::UnsubscribeEntityObserver(
    EntityType entityType, uint32_t expectedDataLength, const sptr<IEntityRecognitionObserver> &observer)
{
    return entityRecognitionMgr_->UnsubscribeEntityObserver(entityType, expectedDataLength, observer);
}
...
{
    return distributedMgr_->ProcessDelayHtmlEntry(data, targetAppInfo, entry);
}

int32_t PasteboardService::PostProcessDelayHtmlEntry(PasteData &data, const AppInfo &targetAppInfo,
    PasteDataEntry &entry)
{
    return distributedMgr_->PostProcessDelayHtmlEntry(data, targetAppInfo, entry);
}
...
{
    return permissionMgr_->CheckAndGrantRemoteUri(data, appInfo, pasteId, pasteBlock);
}
    ...
    std::string &peerNetId, std::string &peerUdid)
{
    ...
    auto [distRet, distEvt] = distributedMgr_->distributedMgr_->GetValidDistributeEvent(appInfo.userId);
    if (distRet == static_cast<int32_t>(PasteboardError::GET_SAME_REMOTE_DATA)) {
        auto isPasting = distributedMgr_->IsRemoteDataPasting(distEvt);
        if (isPasting) {
            PASTEBOARD_HILOGW(PASTEBOARD_MODULE_SERVICE, "wait remote data, seqId=%{public}u", distEvt.seqId);
            distributedMgr_->WaitRemoteData(distEvt);
        }
    }
    if (distRet != static_cast<int32_t>(PasteboardError::E_OK) ||
        GetScreenStatus(appInfo.userId) != ScreenEvent::ScreenUnlocked) {
        auto currentEvent = distributedMgr_->distributedMgr_->GetCurrentEvent();
        pasteBlock = distributedMgr_->EstablishP2PLinkTask(pasteId, currentEvent);
        ...
        pasteBlock = distributedMgr_->EstablishP2PLinkTask(pasteId, distEvt);
        result = distributedMgr_->distributedMgr_->GetRemoteData(appInfo.userId, distEvt, data, syncTime);
...
{
    auto plugin = distributedMgr_->distributedMgr_->GetClipPlugin();
...
{
    distributedMgr_->GetDelayPasteData(userId, data);
}

int32_t PasteboardService::GetDelayPasteRecord(int32_t userId, PasteData &data)
{
    return distributedMgr_->GetDelayPasteRecord(userId, data);
}
...
{
    return distributedMgr_->EstablishP2PLinkTask(pasteId, event);
}

void PasteboardService::CloseP2PLink(const std::string &networkId)
{
    distributedMgr_->CloseP2PLink(networkId);
}

int32_t PasteboardService::PasteStart(const std::string &pasteId)
{
    if (distributedMgr_->ffrtTimer_) {
        distributedMgr_->ffrtTimer_->CancelTimer(pasteId);
    }
    return ERR_OK;
}
...
int32_t PasteboardService::PasteComplete(const std::string &deviceId, const std::string &pasteId)
{
    ...
    std::lock_guard<std::mutex> tmpMutex(distributedMgr_->p2pMapMutex_);
    distributedMgr_->p2pMap_.ComputeIfPresent(deviceId, [pasteId, deviceId, this](const auto &key, auto &value) {
        value.ComputeIfPresent(pasteId, [deviceId](const auto &key, auto &value) {
            return false;
        });
        if (value.Empty()) {
            distributedMgr_->CloseP2PLink(deviceId);
            return false;
        }
        return true;
    });
...
int32_t PasteboardService::GetRemoteDeviceName(std::string &deviceName, bool &isRemote)
{
    ...
    auto tokenId = IPCSkeleton::GetCallingTokenID();
    auto appInfo = GetAppInfo(tokenId);
    auto event = distributedMgr_->distributedMgr_->GetValidDistributeEvent(appInfo.userId);
#ifdef PB_DEVICE_MANAGER_ENABLE
    DmDeviceInfo remoteDevice;
...
{
    permissionMgr_->RemoveInvalidRemoteUri(grantUris);
}
...
{
    return permissionMgr_->GrantPermission(grantUris, permFlag, isRemoteData, targetBundleName, appIndex);
}
...
{
    return permissionMgr_->GrantUriPermission(grantUris, targetBundleName, isRemoteData, appIndex);
}
...
{
    return permissionMgr_->CheckUriPermission(data, targetBundleAndIndex);
}

bool PasteboardService::IsBundleOwnUriPermission(const std::string &bundleName, Uri &uri)
{
    return permissionMgr_->IsBundleOwnUriPermission(bundleName, uri);
}
...
bool PasteboardService::HasPasteData()
{
    ...
    if (GetScreenStatus(userId) == ScreenEvent::ScreenUnlocked) {
        auto [distRet, distEvt] = distributedMgr_->distributedMgr_->GetValidDistributeEvent(userId);
        if (distRet == static_cast<int32_t>(PasteboardError::E_OK)) {
            return true;
        }
    }
...
{
    return distributedMgr_->HasRemoteUri(data);
}
...
bool PasteboardService::HasRemoteData()
{
    ...
    bool hasRemoteUri = distributedMgr_->HasRemoteUri(data);
    return hasRemoteUri;
}
    ...
    const sptr<IPasteboardDelayGetter> delayGetter, const sptr<IPasteboardEntryGetter> entryGetter)
{
    ...
    if (!(pasteData.IsDelayData())) {
        distributedMgr_->SetDistributedData(appInfo.userId, pasteData);
...
void PasteboardService::SetDataExpirationTimer(int32_t userId)
{
    ...
    distributedMgr_->ffrtTimer_->SetTimer(taskName, task, static_cast<uint32_t>(agedTime_.load()));
}
...
int32_t PasteboardService::GetMimeTypes(std::vector<std::string> &funcResult)
{
    ...
    if (GetScreenStatus(userId) == ScreenEvent::ScreenUnlocked) {
        ...
        if (distRet == static_cast<int32_t>(PasteboardError::E_OK)) {
            ...
            PasteData data;
            int32_t syncTime = 0;
            int32_t ret = distributedMgr_->GetRemoteData(userId, distEvt, data, syncTime);
...
bool PasteboardService::HasDataType(const std::string &mimeType)
{
    ...
    if (GetScreenStatus(userId) == ScreenEvent::ScreenUnlocked) {
        ...
        if (distRet == static_cast<int32_t>(PasteboardError::E_OK)) {
            ...
            PasteData data;
            int32_t syncTime = 0;
            if (distributedMgr_->GetRemoteData(userId, distEvt, data, syncTime) != static_cast<int32_t>(PasteboardError::E_OK)) {
                return false;
            }
        }
    }
...
{
    return distributedMgr_->GetValidDistributeEvent(user);
}
...
int32_t PasteboardService::GetRemoteMimeTypes(std::vector<std::string> &mimeTypes, const Event &event)
{
    ...
    auto clipPlugin = distributedMgr_->GetClipPlugin();
...
{
    return distributedMgr_->GetDistributedData(event, user);
}
...
{
    return distributedMgr_->ProcessRemoteDelayUri(deviceId, appInfo, data, record, entry);
}
...
{
    return distributedMgr_->ProcessRemoteDelayHtml(remoteDeviceId, appInfo, rawData, data, record, entry);
}
...
void PasteBoardCommonEventSubscriber::HandleUserSwitched(const EventFwk::CommonEventData &data)
{
    ...
    if (pasteboardService_ != nullptr) {
        ...
        pasteboardService_->distributedMgr_->switch_.DeInit();
        pasteboardService_->distributedMgr_->switch_.Init(context.userId);
...
void PasteboardService::ClearUriOnUninstall(std::shared_ptr<PasteData> pasteData)
{
    ...
    std::thread thread([pasteData, this]() {
        ...
        auto clipPlugin = distributedMgr_->GetClipPlugin();
        PASTEBOARD_CHECK_AND_RETURN_LOGE(clipPlugin != nullptr, PASTEBOARD_MODULE_SERVICE, "clipPlugin is null");
        clipPlugin->Clear(pasteData->userId_);
    });
...
void PasteboardService::PasteboardEventSubscriber()
{
    EventCenter::GetInstance().Subscribe(PasteboardEvent::DISCONNECT, [this](const OHOS::MiscServices::Event &event) {
        ...
        std::lock_guard<std::mutex> tmpMutex(distributedMgr_->p2pMapMutex_);
        distributedMgr_->p2pMap_.EraseIf([networkId, this](auto &key, auto &value) {
            if (key == networkId) {
                distributedMgr_->CloseP2PLink(networkId);
                return true;
            }
...
int32_t PasteboardService::AppExit(pid_t pid, int32_t userId)
{
    ...
    {
        std::lock_guard<std::mutex> tmpMutex(distributedMgr_->p2pMapMutex_);
        distributedMgr_->p2pMap_.EraseIf([pid, &networkIds, this](auto &networkId, auto &pidMap) {
    ...
    for (const auto &id : networkIds) {
        distributedMgr_->CloseP2PLink(id);
    }
...
void PasteboardService::RemotePasteboardChange()
{
    return distributedMgr_->RemotePasteboardChange();
}
